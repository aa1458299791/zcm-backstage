				...
				<el-tabs v-model="activeindex" tab-position="left" :before-leave="befpreTableave"
					...
					<el-tab-pane label="基本信息" name="0">
						...
						<el-form-item label="商品数量" prop="goods_number">
							<el-input v-model.numbe="addForm.goods_number"></el-input>
						</el-form-item>
						<el-form-item label="商品分类" prop="goods_number">
							<!-- options数据来源对象  props配置对象-->
							<el-cascader v-model="addForm.goods_cat" :options="cateList" :props="csaacderprops"
	...
	export default {
		...
		methods: {
			...
			handlesuccess(response) {
				//要往表单数据addForm.pics添加图片的临时地址  用于绑定
			...
			addGoods() {
				...
				this.$refs.addruleFormref.validate(async valid => {
					...
					//this.addForm.goods_cat = this.addForm.goods_cat.join(',') //导致级联选择器关联的goos_cat发生异常
					//需要重新定义一个新的表单对象
					//const form = addForm  //浅拷贝 - 有关联
					//可以使用lodash工具 进行深拷贝
					const form = _.cloneDeep(this.addForm) //使用cloneDeep进行深拷贝
		...
		computed: {
			//判断三级分类
			catId() {
				//判断从后台获取的分类列表addForm里面的数组长度是不是等于3，如果是就返回数组的第三个元素 id，不是就返回空
